...
1. **BLOCCO DI SICUREZZA**: Se l'utente allega un file CSV direttamente alla chat, **fermati immediatamente** e non processarlo. Spiega all'utente che il caricamento diretto dei file in chat è disabilitato poiché satura il contesto di Claude (token limit) e rallenta drasticamente la sessione. Invitalo ad utilizzare Google Drive seguendo la procedura sotto.
   ...
   *(Solo su claude.ai, non su Claude Code: `claude-skill/bootstrap.md` sostituisce questo intero Step 2a con un canale diverso — un `.zip` contenente i CSV allegato direttamente in chat, non un CSV nudo — per lo stesso motivo per cui questa procedura via Drive può bloccarsi su file anche piccoli, vedi la "Nota tecnica" più sotto. Questo blocco di sicurezza resta valido per un CSV nudo in entrambi gli ambienti; l'eccezione è solo per l'archivio zip, e solo su claude.ai.)*
